...
func MatchTransferBalanceChanges(transfer debug.Transfer, tobinTax *TobinTax) gtypes.GomegaMatcher {
	...
	if tobinTax.IsDefined() {
		expectedLen = 4
		res := tobinTax.Apply(transfer.Value)
		balanceChangeMatchers = append(
			GetTransferBalanceChangeMatchers(transfer.From, transfer.To, res.AfterTaxAmount),
			GetTransferBalanceChangeMatchers(transfer.From, tobinTax.Recipient, res.TaxAmount)...)
	}
...
func TestReconcileLockedGoldTransfers(t *testing.T) {
	...
	getTestLockedGoldOps := func(tobinTax *TobinTax) []Operation {
		return []Operation{
			*NewLockGold(address1, address2, amount1, tobinTax),
			*NewUnlockGold(address1, amount1), // does not require transfer
			*NewWithdrawGold(address1, address2, amount1, tobinTax),
			*NewSlash(address1, address2, slashRewardFrom, slashRewardTo, slashPenalty, slashReward, tobinTax),
		}
	}

	getTestTransferOps := func(tobinTax *TobinTax) []Operation {
		return []Operation{
			*NewTransfer(address1, address2, amount1, tobinTax, true),                                                 // matches LockGoldOp
			*NewTransfer(address2, address1, amount1, tobinTax, false),                                                // matches WithdrawGoldOp
			...
			*NewTransfer(address2, address3, amount2, tobinTax, false),                                                // does not match a lockedGoldOp
		}
	}
	...
	t.Run("Without tobinTax", func(t *testing.T) {
		Ω(ReconcileLockedGoldTransfers(lockedGoldOps, transferOps)).Should(ConsistOf(getReconciledOps(lockedGoldOps, emptyTobinTax)))
	})

	t.Run("With tobinTax", func(t *testing.T) {
		Ω(ReconcileLockedGoldTransfers(lockedGoldOpsWithTobinTax, transferOpsWithTobinTax)).Should(ConsistOf(getReconciledOps(lockedGoldOpsWithTobinTax, tobinTax)))
	})
}
